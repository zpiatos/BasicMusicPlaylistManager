...
class MusicPlaylistManager: # PABAGO NG PATH TAS KEEP NIYO YUNG "user_infos.txt"
    ...
    def main_menu(self, user):
        ...
        while True:
            ...
            if choice == "a": 
                ...
                if user.playlist and self.delete_playlist(user):
                    user.playlist = self.create_playlist()
    ...
    def run(self):
        while True:
            print("\nWelcome to Music Playlist Manager! (v2)")
            print("Choices:")
            print("1. Sign Up")
            print("2. Log In")
            print("3. Exit")
            ...
            if choice == "1":
                ...
                print("Exiting MusicPlaylist Manager. Thank you!")
                if len(self.users) > 0:
                    user = self.users[-1]
                    if user.playlist:
                        self.save_playlist(user)
    ...
    def delete_playlist(self, user):
        if not user.playlist:
            print("You don't have a playlist yet.")
            return False

        while True:
            confirmation = input(f"Are you sure you want to delete the playlist '{user.playlist.name}'? (y/n): ").lower()
            if confirmation in ("y", "n"):
                break
            else:
                print("Invalid input. Please enter 'y' for yes or 'n' for no.")

        if confirmation == 'y': # gawa kayo "playlists" folder sa loob ng main folder, tas paltan niyo ng path
            filename = os.path.join("C:\\Users\\Admin\\Desktop\\Music Playlist Manager\\playlists",
                                    f"{user.username}_playlist_{self.version}.txt")
            if os.path.exists(filename):
                try:
                    os.remove(filename)
                    self.playlist_name = user.playlist.name
                    user.playlist = None
            ...
            print("Deleting playlist not continued.")
            return False
